...
build: *.go
	...
	$(GO) build -C cmd/ $(GOFLAGS) $(LDFLAGS) -o '../$(DESTDIR)/$(CLI)'
...
dist: *.go
	...
	# hardened
	GOOS=openbsd GOARCH=amd64 $(GO) build -C cmd/ $(GOFLAGS) $(LDFLAGS) -o '../$(DESTDIR)/opinions-openbsd_amd64-hardened'
	GOOS=linux GOARCH=amd64 $(GO) build -C cmd/ $(GOFLAGS) $(LDFLAGS) -o '../$(DESTDIR)/opinions-linux_amd64-hardened'
	# without sandbox
	GOFLAGS='-tags=unsafe' GOOS=linux GOARCH=arm GOARM=7 $(GO) build -C cmd/ $(GOFLAGS) $(LDFLAGS) -o '../$(DESTDIR)/opinions-linux_armv7'
	GOFLAGS='-tags=unsafe' GOOS=linux GOARCH=arm64 $(GO) build -C cmd/ $(GOFLAGS) $(LDFLAGS) -o '../$(DESTDIR)/opinions-linux_arm64'
	GOFLAGS='-tags=unsafe' GOOS=freebsd GOARCH=amd64 $(GO) build -C cmd/ $(GOFLAGS) $(LDFLAGS) -o '../$(DESTDIR)/opinions-freebsd_amd64'
	GOFLAGS='-tags=unsafe' GOOS=windows GOARCH=amd64 $(GO) build -C cmd/ $(GOFLAGS) $(LDFLAGS) -o '../$(DESTDIR)/opinions-windows_amd64.exe'
...
install-dependencies:
	...
	@GOFLAGS='-v -x' $(GO) get -C cmd/ $(GOFLAGS) .
